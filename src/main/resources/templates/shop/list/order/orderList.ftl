...
<head>
    ...
    <link rel="shortcut icon" href="https://lg-qrm18qcm-1255940368.cos.ap-shanghai.myqcloud.com/bitbug_favicon.ico"
          type="image/x-icon"/>
    <link href="${base}/resources/shopCss/orderStyle.css" rel="stylesheet" type="text/css"/>
...
<div class="weui-search-bar" id="searchBar">
    <form class="weui-search-bar__form" action="${base}/shop/orderListPage/init">
            ...
            <div class="tab-panel">
                <div class="tab-panel-item tab-active news-list" id="newsList">
                    <#list array as array>
                        ...
                        <a href="javascript:openAction('${array.orderId}','${array.frontMoney}','${array.secondMoney}',
                           ...
                           class="aui-content-list">
                            <div class="aui-flex b-line">
                                ...
                                <div class="aui-flex-box">
                                    <h2>${array.goodMapperBeans[0].goodTitle+'<br/>'+array.goodMapperBeans[0].goodBrief}</h2>

                                    <span style="transform: scale(0.9);width: 100%">${'全款:'+array.goodMapperBeans[0].retailPrice+' 定金:'+array.goodMapperBeans[0].frontMoney+' 尾款:'+array.goodMapperBeans[0].secondMoney}</span>
                                    <div class="aui-flex b-line" style="padding: 5px;">
                                        ...
                                        <div class="aui-flex-box">
                                            <h2>${array.userMapperBean.nickName}</h2>
                                        </div>
                                    </div>
...
<script type="text/javascript" charset="utf-8">
    ...
    function openAction(orderId, oneMoney, twoMoney, address, check0, check1,
                        check2,
                        check3, check4,
                        check5, check6) {
        $.actions({
            ...
            actions: [
                {
                    text: "取消订单",
                    onClick: function () {
                        $.confirm("是否确定取消订单", "请确认", function () {
                            $.showLoading();
                            $.post('${base}/shop/orderListPage/cancelOrderPre', {
                                orderId: orderId,
                                status: '-1',
                            }, function (res) {
                                $.hideLoading();
                                if (res.msgCode == 10000) {
                                    $.toptip('操作成功', 'success');
                                    setTimeout(function () {
                                        window.location.replace('${base}/shop/orderListPage/mineOrder');
                                    }, 1000)
                                } else {
                                    $.alert('操作失败,请稍后重试')
                                }
                ...
                <#--    text: "变更为付定金",-->
                <#--    onClick: function () {-->
                <#--        $.confirm("是否确定变更订单状态为付定金", "请确认", function () {-->
                <#--            $.showLoading();-->
                <#--            $.post('${base}/shop/orderListPage/statusPart', {-->
                <#--                orderId: orderId-->
                <#--            }, function (res) {-->
                <#--                $.hideLoading();-->
                <#--                if (res.msgCode == 10000) {-->
                ...
                <#--},-->
                {
                    text: "变更为付全款状态",
                    onClick: function () {
                        $.confirm("是否确定变更订单状态为付全款", "请确认", function () {
                            $.showLoading();
                            $.post('${base}/shop/orderListPage/statusAllPre', {
                                orderId: orderId,
                                status: "1",
                            }, function (res) {
                                $.hideLoading();
                ...
                <#--            onOK: function (text) {-->
                <#--                $.showLoading();-->
                <#--                $.post('${base}/shop/orderListPage/modOne', {-->
                <#--                    orderId: orderId,-->
                <#--                    money: text-->
                <#--                }, function (res) {-->
                <#--                    $.hideLoading();-->
                ...
                <#--            onOK: function (text) {-->
                <#--                $.showLoading();-->
                <#--                $.post('${base}/shop/orderListPage/modTwo', {-->
                <#--                    orderId: orderId,-->
                <#--                    money: text-->
                <#--                }, function (res) {-->
                <#--                    $.hideLoading();-->
                ...
                <#--},-->
                {
                    ...
                    onClick: function () {
                        ...
                        if (check0 != undefined && check0 != 'undefined' && check0) {
                            serverInfo = serverInfo + '选择养宠医师 '
                        }
                        if (check1 != undefined && check1 != 'undefined' && check1) {
                            serverInfo = serverInfo + '选择1年全保换 '
                        }
                        if (check2 != undefined && check2 != 'undefined' && check2) {
                            serverInfo = serverInfo + '选择感染猫瘟热保3月 '
                        }
                        if (check3 != undefined && check3 != 'undefined' && check3) {
                            serverInfo = serverInfo + '选择感染猫传染性腹膜炎保3月 '
                        }
                        if (check4 != undefined && check4 != 'undefined' && check4) {
                            serverInfo = serverInfo + '选择意外死亡保1月 '
                        }
                        if (check5 != undefined && check5 != 'undefined' && check5) {
                            serverInfo = serverInfo + '选择30天疾病报销 '
                        }
                        if (check6 != undefined && check6 != 'undefined' && check6) {
                            serverInfo = serverInfo + '选择1年365天疾病报销 '
                        }
                        // $.alert('用户地址：<br/>' + address + "<br/>选择服务：<br/>" + serverInfo, "详细信息");
                        $.alert('用户地址：<br/>' + address, "详细信息");
                    }
                }
            ]
        });
    ...
    function getListDataFromNet(whichPageNumParams, successCallback, errorCallback) {
        $.showLoading();
        $.post('${base}/shop/orderListPage/dym', {
