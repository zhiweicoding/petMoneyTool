...
<div class="page__pd">
    <div class="weui-search-bar" id="searchBar">
        <div class="weui-search-bar__form" action="${base}/shop/searchPage/searchAny">
...
<script>
    ...
    function submit() {
        ...
        $.showLoading();
        $.post('${base}/shop/searchPage/searchAny', {
            anyText: anyText,
        }, function (res) {
            $.hideLoading();
            ...
            var array = msgBody['array']
            var htmlBodyTotal = ''
            for (var index in array) {
                var item = array[index]
                var html = '<div class="weui-media-box weui-media-box_text" onclick="copyFun(this);"> ' +
                    '<h4 class="weui-media-box__title">猫舍：' + item.weappShopName + '<br/> 上传时间：' + item.createTimeStr + '</h4> ' +
                    '<p class="weui-media-box__desc">' + item.orgDesc + '</p>' +
